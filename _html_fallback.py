...
"""Template HTML interno usado como painel (servido via HTTP)."""
...
async function pollMuteBadge() {
  try {
    const r = await fetch(CTRL + '/mute_status', { cache: "no-store" });
...
async function hydrateWindPref() {
  try {
    const r = await fetch(CTRL + '/wind_pref', { cache: "no-store" });
    const j = await r.json();
...
async function setWindPref(val) {
  try { await fetch(CTRL + '/wind_pref?host=' + encodeURIComponent(val), { cache: "no-store" }); } catch (e) {}
...
@media (max-width: 1200px) {
  ...
  .wind-main, .wind-secondary { justify-self: stretch; transform: none; }
  .status-indicator, .controls, .main-panel { transform: none; }
}
...
@media (max-width: 768px) {
  ...
  .main-panel { padding: 1.5rem 2rem; transform: none; }
  .main-status { font-size: 4rem; }
  .vento-valor { font-size: 2.5rem; }
  .rajada-valor { font-size: 3rem; }
  .status-indicator, .controls { transform: none; }
  .wind-main, .wind-secondary { transform: none; }
}
</style>
</head>
